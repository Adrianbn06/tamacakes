            ...
            if (timestamp && timestamp > lastSyncTime) {
                ...
                setTimeout(() => {
                    if(currentUser && currentUser.email) {
                        cargarDatos(currentUser.email, true); // true = Modo silencioso
                    }
                }, 1500);
            }
        });
    } catch(e) { console.warn("Firebase en vivo inactivo:", e); }
}

function tocarTimbre() {
    try {
        lastSyncTime = Date.now();
        // Si Firebase falla, el .catch evita que el error rompa tu aplicación
        firebase.database().ref('timbre_actualizacion').set(lastSyncTime).catch(e => console.warn(e));
    } catch(e) { console.warn("No se pudo tocar el timbre:", e); }
}
...
async function apiCall(action, payload={}) {
  ...
  // Aumentamos el tiempo a 20 segundos por si el internet de la pastelería está lento
  ...
  try {
    ...
    if (error.name === 'AbortError') {
        UI.showToast('⏳ Tiempo de espera agotado. Revisa tu internet.', 'error');
    } else if (error.message === 'Token de seguridad inválido') {
        UI.showToast('🔒 Error: Acceso Denegado por el Servidor.', 'error');
    } else {
...
function cargarDatos(email, silencioso = false) {
  ...
  else setSyncing(true, 'Actualizando...');
  ...
  apiCall('getAll', { email }).then(data => {
    ...
    // ELIMINADO: El toast amarillo de "Pantalla actualizada" ya no aparecerá.
