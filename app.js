...
// ============================================================================
// 🎨 CAPA 2: CONTROLADORES DE INTERFAZ (VISTA Y SEGURIDAD)
// ============================================================================

// 🔄 NUEVA FUNCIÓN: Recargar Base de Datos
...
// 🔒 NUEVA FUNCIÓN: Sistema de Roles y Permisos
function aplicarRoles() {
    const rol = currentUser?.rol ? String(currentUser.rol).toLowerCase() : 'empleado';
    
    // Tamara tiene exactamente el mismo poder que el admin
    const esAdmin = (rol === 'admin' || rol === 'tamara');
    ...
    // Nueva Venta y Registros son visibles para Admin y Cajero
    document.getElementById('tabBtnVen').style.display = (esAdmin || esCajero) ? 'inline-block' : 'none';
    document.getElementById('tabBtnHis').style.display = (esAdmin || esCajero) ? 'inline-block' : 'none';

    // Bloqueos de redirección
    if(!esAdmin && !esCajero) {
        // Es un pastelero: Forzamos a ver únicamente la cocina
        switchTab('tar', document.getElementById('tabBtnTar'));
        const selFiltro = document.getElementById('filtroCocinaEncargado');
        if(selFiltro && rol !== 'empleado') {
        ...
        // Es un cajero: Si está en una pestaña no permitida, lo mandamos a ventas
        const panelActivo = document.querySelector('.tab-pane.active').id;
        if(panelActivo === 'tab-inv' || panelActivo === 'tab-rct' || panelActivo === 'tab-pro') {
            switchTab('nuevaventa', document.getElementById('tabBtnVen'));
        }
    }
}
...
function renderTodo() { 
    ...
    aplicarRoles(); // Ejecutamos la seguridad al renderizar
}
...
function updateFiltroCocina() {
  ...
  if(!sel) return;
  // Solo actualizamos las opciones si no está deshabilitado (los pasteleros lo tienen deshabilitado)
  if(!sel.disabled) {
...
function renderTar() {
  ...
  // Verificar si es Cajero para bloquear los selectores
  const rol = currentUser?.rol ? String(currentUser.rol).toLowerCase() : 'empleado';
  ...
  tb.innerHTML = sorted.map(t=>{
    ...
    // NUEVO: Bloquear selector si la tarea está completada o si el usuario es Cajero
    const disableSelect = t.estado === 'Completado' || isCajero ? 'disabled' : '';
