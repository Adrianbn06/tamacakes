...
const BusinessLogic = {
  ...
  Ventas: {
      agendar: async (c, cel, fE, fP, m, stP, notas, ad, items) => {
          ...
          await dbUpsert('Pedidos', nuevoPedido);
          if (nuevasTareas.length > 0) {
              await new Promise(resolve => setTimeout(resolve, 600));
              await dbUpsert('Tareas', nuevasTareas);
          }
      },
  ...
  Cocina: {
      cambiarEstado: async (id, st) => {
          ...
          if(st === 'Completado' && t.estado !== 'Completado') {
              ...
              t.estado = 'Completado';
              if(updatedMats.length > 0) await dbUpsert('Inventario', updatedMats);
          } else {
              t.estado = st; 
          }

          if(st === 'Completado') await new Promise(r => setTimeout(r, 400));
          await dbUpsert('Tareas', t);
          ...
          if (st === 'Completado' && t.pedido_id) {
              ...
              if (parentOrder && parentOrder.estado !== 'Entregado' && parentOrder.estado !== 'Listo para entrega') {
                  ...
                  if (orderTasks.every(x => x.estado === 'Completado')) { 
                      ...
                      await new Promise(r => setTimeout(r, 400));
                      await dbUpsert('Pedidos', parentOrder); 
...
function recargarDatos() {
    if(currentUser && currentUser.email) {
        UI.showToast('Actualizando datos...', 'success');
        cargarDatos(currentUser.email);
    }
}

function renderTodo() { 
    renderInv(); renderRct(); renderPro(); renderTar(); renderOrd(); 
    updateSelects(); checkAlertas(); updateFiltroCocina(); 
}
...
function mostrarListaCompras() {
    ...
    if (lista.length === 0) {
        ...
        tb.innerHTML = lista.map(item => `<tr><td><strong>${item.nombre}</strong></td><td style="color: var(--rose); font-weight: bold;">${item.falta}</td></tr>`).join('');
    }
    document.getElementById('modalCompras').style.display = 'flex';
}

function generarTextoCompras() {
    const lista = calcularListaCompras();
    if(lista.length === 0) return "🟢 Tienes stock suficiente para todos los pedidos actuales en TamaCakes.";
    
    let texto = "🛒 *LISTA DE COMPRAS - TAMACAKES* 🛒\n\n";
    lista.forEach(item => {
        texto += `▫️ *${item.nombre}:* Falta ${item.falta}\n`;
    });
...
function copiarListaCompras() {
    ...
    navigator.clipboard.writeText(texto).then(() => {
        UI.showToast('Lista copiada al portapapeles', 'success');
    }).catch(err => {
        UI.showToast('Error al copiar. Selecciona el texto manualmente.', 'error');
    });
}

function compartirWhatsAppCompras() {
    const texto = generarTextoCompras();
...
function imprimirRecibo(id) {
  ...
  ord.items.forEach(it => {
    ...
    total += sub;
    itemsHtml += `<tr><td>${it.cantidad}x</td><td>${p ? p.nombre : 'Producto borrado'}</td><td style="text-align:right;">$${sub.toFixed(2)}</td></tr>`;
  });
...
function renderTar() {
  const sel = document.getElementById('filtroCocinaEncargado');
  const filtroVal = sel ? String(sel.value).toLowerCase().trim() : 'todos';
  
  let tareasFiltradas = tareas;
  if(filtroVal !== 'todos') { 
      tareasFiltradas = tareas.filter(t => { 
          const r = recetas.find(x => x.id === t.receta_id); 
          const encargadoReceta = r ? String(r.encargado || '').toLowerCase().trim() : '';
          return encargadoReceta === filtroVal; 
      }); 
  }
  ...
  const nombreEstacionDisplay = sel ? sel.options[sel.selectedIndex].text : 'Todas';

  if(pendientes.length === 0) {
      resumenContainer.innerHTML = `<strong style="color:#9c7a60;">Resumen a preparar (${nombreEstacionDisplay}):</strong> <span style="color:#888;">Estación libre de tareas.</span>`;
  } else {
      ...
      resumenContainer.innerHTML = `<strong style="display:block; margin-bottom:10px; color:var(--brown);">Resumen Total de Estación (${nombreEstacionDisplay}):</strong>` + htmlItems.join('');
  }
  ...
  tb.innerHTML = sorted.map(t=>{
    ...
    const disableSelect = t.estado === 'Completado' ? 'disabled' : '';
    const sel = `<select style="font-family:var(--font-b); border-radius:6px; padding:4px;" onchange="cambiarEstadoTar(${t.id},this.value)" ${disableSelect}>${ESTADOS_TAR.map(e=>`<option ${t.estado===e?'selected':''}>${e}</option>`).join('')}</select>`;
